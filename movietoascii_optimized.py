...
pixel_ascii_map = " .'-~|/+<?vcrenXuOAQ8d$"
length = len(pixel_ascii_map)
margin_pixels = 20
...
def image_to_ascii(image):
    ...
    for x in range(0, im.size[1]):
        ...
        for y in range(0, im.size[0]):
            ...
            ascii_val = pixel_ascii_map[brightness * (length - 1) // 255]
    ...
    image_width = int(ceil(font.getsize(''.join([i[0] for i in lines[0]]))[PIL_WIDTH_INDEX] + (2 * margin_pixels)))
...
# clip = clip.subclip(0, os.environ['DURATION'])
...
for x in range(0, 10):
    print("waiting for" + str(x) + "/30 seconds to make sure the video is ready ")
    time.sleep(1)
